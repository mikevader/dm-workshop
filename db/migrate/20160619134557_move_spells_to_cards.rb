  ...
  def up
    ...
    spells.each do |spell|
      ...
      card_id = insert("INSERT INTO cards (type, name, level, school, casting_time, range, components, duration, short_description, description, user_id, created_at, updated_at, picture, athigherlevel, concentration, ritual, cite, shared)
        VALUES ('#{type}', #{sani(name)}, #{level}, #{sani(school)}, #{sani(casting_time)}, #{sani(range)}, #{sani(components)}, #{sani(duration)}, #{sani(short_description)}, #{sani(description)}, #{user_id}, '#{created_at}', '#{updated_at}', #{sani(picture)}, #{sani(athigherlevel)}, #{sani(concentration)}, #{sani(ritual)}, #{sani(cite)}, #{sani(shared)})")
  ...
  def down
    ...
    spells.each do |spell|
      ...
      spell_id = insert("INSERT INTO spells (name, level, school, casting_time, range, components, duration, short_description, description, user_id, created_at, updated_at, picture, athigherlevel, concentration, ritual, cite, shared)
        VALUES (#{sani(name)}, #{level}, #{sani(school)}, #{sani(casting_time)}, #{sani(range)}, #{sani(components)}, #{sani(duration)}, #{sani(short_description)}, #{sani(description)}, #{user_id}, '#{created_at}', '#{updated_at}', #{sani(picture)}, #{sani(athigherlevel)}, #{sani(concentration)}, #{sani(ritual)}, #{sani(cite)}, #{sani(shared)})")
